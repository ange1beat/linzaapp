...
def test_request_id_set_via_middleware(client):
    """RequestIDMiddleware extracts X-Request-ID and sets ContextVar."""
    resp = client.get("/health", headers={"X-Request-ID": "debug-req-42"})
    assert resp.status_code == 200
    assert resp.headers.get("x-request-id") == "debug-req-42"
