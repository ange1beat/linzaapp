...
public abstract class ReflectedMedia : IMedia
{
    public virtual void Write<T>(string name, T value)
    {
        ...
        if (propertyInfo.GetSetMethod(true) is null)
        {
            return;
        }
    ...
    {
        if (value is IWritable ^ typeof(IMedia).IsAssignableFrom(propertyInfo.PropertyType))
        {
            throw new InvalidOperationException(
                "Failed to write Media. Either property is not IMedia or the value is not IWritable." +
